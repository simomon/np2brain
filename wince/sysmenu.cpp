...
static void sys_cmd(MENUID id) {
	...
	switch(id) {
		...
		case MID_QWERTYDEF:
			winkbd_bindqwerty(0);
			np2oscfg.bindqwerty = 0;
			update |= SYS_UPDATEOSCFG;
			break;

		case MID_QWERTY1:
			winkbd_bindqwerty(1);
			np2oscfg.bindqwerty = 1;
			update |= SYS_UPDATEOSCFG;
			break;

		case MID_QWERTY2:
			winkbd_bindqwerty(2);
			np2oscfg.bindqwerty = 2;
			update |= SYS_UPDATEOSCFG;
			break;
#endif
		case MID_KEY:
...
#endif
#if defined(BRAIN)
	b = np2oscfg.bindqwerty;
	menusys_setcheck(MID_QWERTYDEF, (b == 0));
	menusys_setcheck(MID_QWERTY1, (b == 1));
	menusys_setcheck(MID_QWERTY2, (b == 2));
#endif
	b = np2cfg.KEY_MODE;
